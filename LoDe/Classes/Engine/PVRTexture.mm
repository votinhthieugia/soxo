...
- (BOOL)createGLTexture {
	...
	if ([_imageData count] != 1) {
		[NSException raise:NSInternalInconsistencyException format:@"Invalid PVR imageData count"];
	}
	...
	// TODO: Is GL_NEAREST faster here?
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);

	data = [_imageData objectAtIndex:0];
	glCompressedTexImage2D(GL_TEXTURE_2D, 0, _internalFormat, width, height, 0, [data length], [data bytes]);
